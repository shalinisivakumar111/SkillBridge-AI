...
def render_map(candidate_lat, candidate_lon, results):
    ...
    for _, row in results.iterrows():
        ...
        location = html.escape(row.get("address") or row.get("location") or "Location not provided")
        dist = float(row["distance_km"])
        score = float(row["final_score"])
        matched = row.get("skill_matches", []) or []
        gaps = row.get("skill_gaps", []) or []
        matched_text = ", ".join(
            html.escape(str(m.get("job_skill") or m.get("candidate_skill") or ""))
            for m in matched[:4]
        ) or "No structured skill matches"
        gap_text = ", ".join(html.escape(str(g)) for g in gaps[:4]) or "No detected gap"
        apply_url = html.escape(str(row.get("redirect_url") or ""), quote=True)

        apply_html = (
            f'<br><a href="{apply_url}" target="_blank">🔗 View / Apply</a>'
            if apply_url else
            "<br><span>No application link in source</span>"
        )
        ...
        popup_html = (
            ...
            f"📍 {location}<br>"
            f"🧭 {dist:.2f} km<br>"
            f"🎯 Recommendation: <b>{score:.1f}%</b><br>"
            f"🟢 Skills: {matched_text}<br>"
            f"🧩 Gap: {gap_text}"
            f"{apply_html}"
            f"</div>"
        )
...
if st.button(f"🔎 {T['find']}", type="primary", width="stretch"):
    ...
    # ========================================================
    # EXPLAINABLE RECOMMENDATION STORY
    # ========================================================
    if judge_mode:
        st.divider()
        st.header("🧭 How SkillBridge Solves the Problem")

        s1, s2, s3 = st.columns(3)
        with s1:
            st.markdown("### 1️⃣ Understand")
            st.write(
                f"**{candidate_name or 'Candidate'}** is searching for "
                f"**{profession}** using **{len(candidate_skills)} skill(s)**."
            )
            st.caption(
                f"Location: {candidate_location_name or 'coordinates'} • "
                f"Radius: {radius} km"
            )

        with s2:
            st.markdown("### 2️⃣ Match Locally")
            st.write(
                f"SkillBridge evaluated jobs with valid coordinates and kept "
                f"**{len(results_df)} local opportunities** inside the selected radius."
            )
            st.caption(
                "Role relevance + skill evidence + Haversine distance"
            )

        with s3:
            st.markdown("### 3️⃣ Improve")
            preview_gaps = aggregate_skill_gaps(
                results_df, candidate_skills, limit=3
            )
            if preview_gaps:
                st.write(
                    "Prioritizes skills that appear repeatedly in the recommended jobs: "
                    + ", ".join(item["skill"].title() for item in preview_gaps)
                    + "."
                )
            else:
                st.write(
                    "No repeated skill gap was detected in the current recommendation pool."
                )
            st.caption("Recommendations are evidence-based, not hiring guarantees.")
    ...
    # A compact, explainable impact summary for demos/judging.
    best_row = results_df.iloc[0]
    st.info(
        f"🏆 **Best local opportunity:** {best_row['job_title']} at {best_row['company']} — "
        f"{best_row['final_score']:.1f}% recommendation, {best_row['distance_km']:.2f} km away. "
        "Scores are ranking signals based on the available role, skill, and location evidence."
    )
    ...
    if judge_mode:
        ...
        with e1:
            st.metric("Role Evidence", f"{float(best_row['role_match']):.1f}%")
        with e2:
            st.metric("Skill Evidence", f"{float(best_row['skill_match']):.1f}%")
        with e3:
            st.metric("Distance", f"{float(best_row['distance_km']):.2f} km")
        with e4:
            gap_count = len(best_row["skill_gaps"])
            st.metric("Skill Gaps", str(gap_count))

        st.caption(
            "Ranking formula: 40% role relevance + 40% skill evidence + "
            "20% local proximity. Missing structured job skills reduce confidence; "
            "the system does not invent skills or distances."
        )

        st.markdown("### 💡 Why This Job?")
        why_bits = []
        role_value = float(best_row["role_match"])
        skill_value = float(best_row["skill_match"])
        distance_value = float(best_row["distance_km"])

        if role_value >= 70:
            why_bits.append(f"Strong role alignment ({role_value:.0f}%).")
        elif role_value >= 40:
            why_bits.append(f"Moderate role alignment ({role_value:.0f}%).")
        else:
            why_bits.append(f"Limited role alignment ({role_value:.0f}%).")

        if skill_value >= 70:
            why_bits.append(f"Strong evidence from candidate skills ({skill_value:.0f}%).")
        elif skill_value >= 40:
            why_bits.append(f"Partial skill alignment ({skill_value:.0f}%).")
        else:
            why_bits.append(f"Several skills may need improvement ({skill_value:.0f}%).")

        if distance_value <= 5:
            why_bits.append(f"Very close local opportunity ({distance_value:.2f} km).")
        elif distance_value <= 15:
            why_bits.append(f"Reasonably local ({distance_value:.2f} km).")
        else:
            why_bits.append(f"Further away ({distance_value:.2f} km), so compare alternatives.")

        st.write(" ".join(why_bits))

        gaps = best_row["skill_gaps"]
        if gaps:
            st.warning(
                "Priority skill gap(s): "
                + ", ".join(str(g).title() for g in gaps[:3])
                + "."
            )
        else:
            st.success("No explicit skill gap was detected for this job from the available data.")
    ...
    if judge_mode and not family_counts.empty:
        st.markdown("### 📊 Local Demand Signal")
        # family_counts is already a DataFrame with:
        #   Job Family | Jobs
        # Convert the family column into the chart index instead of
        # renaming the entire two-column axis (which causes a pandas
        # "Length mismatch" error).
        chart_df = family_counts.set_index("Job Family")[["Jobs"]]
        st.bar_chart(chart_df, width="stretch")
        st.caption(
            "This chart describes the current jobs returned by the search pool; "
            "it is not a forecast of the entire local labor market."
        )
    ...
    st.caption(
        ...
        "Click a job marker to see the company, distance, match evidence, skill gap, and application link. "
    ...
    quick = results_df[
        ["job_title", "company", "distance_km", "role_match", "skill_match", "final_score"]
    ].head(5).copy()
    quick.columns = [
        "Job",
        "Company",
    ...
    quick["Distance (km)"] = quick["Distance (km)"].map(lambda x: f"{float(x):.2f}")
    quick["Role Match"] = quick["Role Match"].map(lambda x: f"{float(x):.1f}%")
    quick["Skill Match"] = quick["Skill Match"].map(lambda x: f"{float(x):.1f}%")
    quick["Recommendation"] = quick["Recommendation"].map(lambda x: f"{float(x):.1f}%")
    st.dataframe(quick, width="stretch", hide_index=True)

    applications_available = int(
        results_df["redirect_url"].fillna("").astype(str).str.strip().ne("").sum()
    )
    closest_job = results_df.iloc[0]
    closest_distance = float(results_df["distance_km"].min())
    best_score = float(results_df["final_score"].max())

    h1, h2, h3, h4 = st.columns(4)
    with h1:
        st.metric("Local Opportunities", f"{len(results_df)}")
    with h2:
        st.metric("Closest Opportunity", f"{closest_distance:.2f} km")
    with h3:
        st.metric("Best Recommendation", f"{best_score:.1f}%")
    with h4:
        st.metric("Application Links", f"{applications_available}/{len(results_df)}")

    st.caption(
        "The recommendation percentage is an explainable ranking signal based on "
        "profession relevance, structured skill evidence and local distance. "
        "It is not a hiring probability."
    )

    # Let judges/users take the recommendation results with them.
    export_columns = [
        "job_id", "job_title", "company", "address",
        "distance_km", "role_match", "skill_match", "final_score",
        "family", "role", "redirect_url", "source_type",
    ]
    export_df = results_df[
        [c for c in export_columns if c in results_df.columns]
    ].copy()

    st.download_button(
        "⬇️ Download Recommended Jobs (CSV)",
        data=export_df.to_csv(index=False).encode("utf-8-sig"),
        file_name="skillbridge_recommended_local_jobs.csv",
        mime="text/csv",
        width="stretch",
    )

    st.caption(
        "Use the map for geographic context, the comparison above for fast ranking, "
        "and the detailed cards below for skill-gap and application information."
    )
    st.caption(
    ...
    if recommendations:
        ...
        st.markdown("### 📈 Skill Improvement Impact")
        st.caption(
            "This is an opportunity signal, not a promise of hiring. It counts how many "
            "currently recommended jobs require each missing skill."
        )
        impact_cols = st.columns(min(3, len(recommendations)))
        for col, item in zip(impact_cols, recommendations[:3]):
            with col:
                st.metric(
                    item["skill"].title(),
                    f"{item['jobs']} local job(s)",
                    delta=f"{item['coverage']:.0f}% of recommendations",
                )
                st.caption(
                    f"Priority: {item['priority']} • {item['tip']}"
                )
    ...
    if judge_mode:
        st.divider()
        st.header("🛡️ Why the Recommendation Is Trustworthy")
        t1, t2, t3 = st.columns(3)
        with t1:
            st.markdown("**📍 Local-first**")
            st.write("Distance is calculated from candidate and job coordinates.")
        with t2:
            st.markdown("**🧠 Explainable**")
            st.write("Each recommendation exposes role, skill and distance evidence.")
        with t3:
            st.markdown("**🚫 No fake certainty**")
            st.write("Scores rank available jobs; they do not promise employment.")

    # --------------------------------------------------------
    # ACTION PLAN
    # --------------------------------------------------------
    st.divider()
    st.header("🚀 What the Candidate Should Do Next")

    top_job = results_df.iloc[0]
    top_gaps = aggregate_skill_gaps(results_df.head(min(5, len(results_df))), candidate_skills, limit=3)

    action_items = [
        f"1. Review the top recommendation: **{top_job['job_title']}** at **{top_job['company']}** "
        f"({float(top_job['distance_km']):.2f} km away).",
    ]

    if top_gaps:
        action_items.append(
            "2. Prioritize these high-value skills: "
            + ", ".join(item["skill"].title() for item in top_gaps)
            + "."
        )
    else:
        action_items.append(
            "2. No repeated skill gap was detected across the top local recommendations."
        )

    if applications_available:
        action_items.append(
            f"3. Open one of the **{applications_available}** available application links "
            "and verify the employer requirements before applying."
        )
    else:
        action_items.append(
            "3. No direct application links were supplied by the current job records."
        )
    ...
    with st.expander("🧠 How SkillBridge AI makes recommendations"):
        st.write(
            "1. **Role relevance** — compares the selected profession with the job role/family.\n"
            "2. **Skill evidence** — compares candidate skills with skills detected in the job record.\n"
            "3. **Local relevance** — calculates Haversine distance from the candidate location to the job coordinates.\n"
            "4. **Ranking** — combines the available evidence into a transparent recommendation score.\n"
            "5. **Skill improvement** — identifies missing skills that occur across the recommended local jobs."
        )
        st.caption(
            "The system uses the local job dataset and deterministic matching logic, "
            "so recommendations remain lightweight and explainable."
        )
    if not family_counts.empty:
