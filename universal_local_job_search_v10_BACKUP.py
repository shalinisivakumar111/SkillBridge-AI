...
if st.button(f"🔎 {T['find']}", type="primary", width="stretch"):
    ...
    if judge_mode and not family_counts.empty:
        ...
        chart_df = family_counts.copy()

        # Build a safe chart dataframe regardless of whether
        # Job Family is currently an index or a dataframe column.
        if "Job Family" in chart_df.columns:
            if "Jobs" in chart_df.columns:
                chart_df = chart_df.set_index("Job Family")[["Jobs"]]
            else:
                value_cols = [c for c in chart_df.columns if c != "Job Family"]
                if value_cols:
                    chart_df = chart_df.set_index("Job Family")[[value_cols[0]]]
                    chart_df.columns = ["Jobs"]
                else:
                    chart_df = chart_df.iloc[:, :0]
        elif "Jobs" in chart_df.columns:
            chart_df = chart_df[["Jobs"]]
        elif len(chart_df.columns) >= 1:
            chart_df = chart_df.iloc[:, [0]]
            chart_df.columns = ["Jobs"]

        chart_df.index.name = "Job Family"

        if not chart_df.empty and "Jobs" in chart_df.columns:
