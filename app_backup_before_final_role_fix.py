...
def role_search_score(search_text, row):
    """
    Score how strongly a requested job role matches the actual job title,
    category, description and skills. This fixes searches such as 'doctor',
    'nurse', 'teacher', etc. where the role is not itself a candidate skill.
    """
    ...
    # Common medical/health synonyms.
    synonyms = {
        "doctor": {"doctor", "physician", "medical doctor", "mbbs", "medical officer", "general physician"},
        "physician": {"doctor", "physician", "medical doctor", "mbbs", "medical officer"},
        ...
        "accountant": {"accountant", "accounting", "accounts", "finance"},
        "developer": {"developer", "software developer", "programmer", "engineer", "software engineer"},
    }

    terms = {query}
    terms.update(synonyms.get(query, set()))

    # Exact/strong title match gets the highest score.
    if any(term in title for term in terms):
        return 100.0

    combined = " ".join([category, category_tag, skills, description])
    if any(term in combined for term in terms):
        return 70.0

    # Token overlap supports multi-word roles such as "medical officer".
...
if find_jobs or not st.session_state.last_results.empty:
    if find_jobs:
        ...
        for _, job in jobs_df.iterrows():
            ...
            role_match = role_search_score(job_role_query, job)
            ...
            if job_role_query.strip():
                # Role is the primary signal when the user explicitly requests a job.
                # 60% role relevance + 30% skills + 10% location.
                final_score = min(
                    role_match * 0.60 + skill_match * 0.30 + loc_score * 0.50,
                    100.0,
                )
            else:
        ...
        for idx in results_df.index:
            ...
            if lat is not None and lon is not None:
                ...
                results_df.at[idx, "final_score"] = round(min(float(results_df.at[idx, "skill_match"]) + location_score(d), 100.0), 1)
    ...
    if job_role_query.strip():
        st.write(f"**Requested job role:** {job_role_query}")
