    ...
    if find_jobs:
        ...
        for idx in results_df.index:
            ...
            if lat is not None and lon is not None:
                ...
                role_component = float(results_df.at[idx].get("role_match", 0) or 0)
                skill_component = float(results_df.at[idx].get("skill_match", 0) or 0)
                if effective_role_query:
                    refreshed_score = role_component * 0.60 + skill_component * 0.30 + location_score(d) * 0.50
                else:
