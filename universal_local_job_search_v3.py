...
if st.button(f"🔎 {T['find']}", type="primary", use_container_width=True):
    ...
    family_counts = results_df["family"].value_counts().rename_axis("Job Family").reset_index().rename(columns={"count": "Jobs"})
    skill_demand = {}
    ...
    with d1:
        ...
        if not family_counts.empty:
            st.dataframe(
                family_counts.rename_axis("Job Family").reset_index().rename(columns={"family": "Job Family", "count": "Jobs"}),
    ...
    for i, row in results_df.iterrows():
        with st.container(border=True):
            ...
            with c4:
                st.metric("Recommendation", f"{row['final_score']:.1f}%")
