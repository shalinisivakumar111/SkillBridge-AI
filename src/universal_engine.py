...
# Used only when the real job source does not provide GPS.
# Distance is then clearly labelled as approximate city distance.
# ============================================================
CITY_COORDINATES = {
...
def resolve_job_coordinates(row):
    """Prefer exact source coordinates; otherwise resolve a known city centroid."""
    lat = safe_float(row.get("latitude"))
    lon = safe_float(row.get("longitude"))
    if lat is not None and lon is not None:
        return lat, lon, "Exact job-source coordinates"

    location_text = " ".join([
        clean_value(row.get("location")),
        clean_value(row.get("location_display")),
        clean_value(row.get("address")),
        clean_value(row.get("searched_location")),
    ]).lower()
    ...
    for city in sorted(CITY_COORDINATES, key=len, reverse=True):
        if city in location_text:
            ...
            return lat, lon, f"Approximate {city.title()} city coordinates"

    return None, None, "No coordinates available from source"


def realistic_final_score(role_match, skill_match, location_score_value, role_requested):
    """Calculate a realistic match score and never claim a perfect match."""
    role = max(0.0, min(float(role_match or 0), 95.0))
    skills = max(0.0, min(float(skill_match or 0), 100.0))
    location = max(0.0, min(float(location_score_value or 0), 20.0))

    if role_requested:
        # Role 45%, skills 40%, location 15% (location is a 0-20 signal).
        score = role * 0.45 + skills * 0.40 + (location / 20.0) * 15.0
    else:
        score = skills * 0.80 + (location / 20.0) * 20.0
...
def role_search_score(search_text, row):
    ...
    # IMPORTANT: for profession searches, the JOB TITLE is the gatekeeper.
    # Do not promote a neutral title such as "Project Coordinator" just
    # because its skills/category happen to mention healthcare, doctors,
    # machine learning, etc. That creates false positives.
    #
    # A Doctor search therefore accepts only titles containing a known
    # doctor/physician/medical-specialist title. The same strict rule is
    # applied to the other explicit profession queries handled above.
    strict_profession_queries = {
        "doctor", "physician", "nurse", "dentist", "pharmacist",
        "teacher", "accountant", "developer"
    }
    if query in strict_profession_queries:
        return 0.0

    # For free-form/non-profession searches, a category/skill fallback is
    # still allowed, but only after the title gate above has been checked.
...
if find_jobs or not st.session_state.last_results.empty:
    if find_jobs:
        ...
        for _, job in jobs_df.iterrows():
            job_lat, job_lon, source_location_status = resolve_job_coordinates(job)
            ...
            # IMPORTANT: never discard a relevant job just because the source
            # record has no coordinates. Coordinates are used for ranking when
            # available, but JOB ROLE relevance is more important than location.
            if active_job_lat is not None and active_job_lon is not None:
            ...
            # Location is a ranking signal, not a hard filter for a requested
            # profession. This prevents a Doctor search from collapsing to one
            # job simply because other real Doctor jobs have missing coordinates.
            loc_score = location_score(distance) if distance is not None else 0.0

            final_score = realistic_final_score(
                role_match,
                skill_match,
                loc_score,
                role_requested,
            ...
            scored.append({
                ...
                "distance_label": distance_label(distance) if distance is not None else "Distance unavailable from source job data",
                ...
                "location_status": clean_value(override.get("source")) if override else (clean_value(job.get("location_status")) or source_location_status),
                "company_location_source": clean_value(override.get("source")) if override else source_location_status,
            })

        if not scored:
            st.warning("No relevant jobs with usable location data were found. Jobs with a known city use an approximate city distance; jobs with no usable location are not assigned a fake distance.")
            st.stop()

        # Prefer nearby jobs when coordinates exist, while still allowing
        # relevant real-world jobs whose source did not provide coordinates.
        results_df["distance_sort"] = results_df["distance_km"].fillna(10**9)
        results_df = results_df.sort_values(["final_score", "distance_sort"], ascending=[False, True]).head(10).reset_index(drop=True)
        results_df.drop(columns=["distance_sort"], inplace=True, errors="ignore")
        ...
        for idx in results_df.index:
            ...
            if lat is not None and lon is not None:
                ...
                d = None
                results_df.at[idx, "distance_km"] = None
                results_df.at[idx, "distance_label"] = "Distance unavailable — source has no usable location"
                results_df.at[idx, "location_score"] = 0.0
            ...
            loc_component = location_score(d) if d is not None else 0.0
            refreshed_score = realistic_final_score(
                role_component,
                skill_component,
                loc_component,
                bool(effective_role_query),
            )
            results_df.at[idx, "final_score"] = refreshed_score
        ...
        results_df["distance_sort"] = results_df["distance_km"].fillna(10**9)
        results_df = results_df.sort_values(["final_score", "distance_sort"], ascending=[False, True]).drop(columns=["distance_sort"], errors="ignore").reset_index(drop=True)
        st.session_state.last_results = results_df.copy()
    st.divider()
    ...
    st.caption("Full job details, company location controls, matched skills, skill gaps and application links. Relevant jobs are not discarded just because source coordinates are missing.")
    ...
    for number, (_, row) in enumerate(results_df.iterrows(), start=1):
        with st.container(border=True):
            ...
            with c1:
                st.metric("Role Match", f"{row.get('role_match', 0):.1f}%")
            with c2:
                st.metric(T["skill_match"], f"{row['skill_match']:.1f}%")
            with c3:
                st.metric(T["distance"], f"{row['distance_km']:.2f} km" if pd.notna(row["distance_km"]) else "Unavailable")
            with c4:
            ...
            st.caption((f"{row['distance_km']:.2f} km away • " if pd.notna(row["distance_km"]) else "Distance unavailable • ") + row["distance_label"])
            ...
            st.markdown(f"### ⭐ Match Assessment")
            st.write(f"**{match_label(row['final_score'])}** — {row['final_score']:.1f}% overall match")
            st.caption("The score combines role relevance, skill coverage and location. It is capped below 100% to avoid presenting a false perfect match.")
            ...
            distance_sentence = (f"This job is {row['distance_km']:.2f} km away." if pd.notna(row["distance_km"]) else "Distance is unavailable from the source job data.")
            st.info(f"{strength}. {distance_sentence} Overall recommendation score: {row['final_score']:.1f}%.")
            ...
            if row["redirect_url"]:
                st.link_button("🔗 APPLY FOR THIS JOB", row["redirect_url"], use_container_width=True)
            else:
                st.info("Application link is not available in the original job source.")

            st.caption("Doctor/role results are searched within 300 km so the app can show multiple relevant opportunities when the local area has only one listing. Address and coordinates are displayed from the real job-source data. SkillBridge does not invent missing floor, landmark, street or building details.")
    ...
    for number, (_, row) in enumerate(results_df.iterrows(), start=1):
        quick_rows.append({
            ...
            "Distance": (f"{row['distance_km']:.2f} km" if pd.notna(row["distance_km"]) else "Unavailable"),
